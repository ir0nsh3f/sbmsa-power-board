(function(root,factory){
  ...
  function profiles(data,sport){
    const all=R.compute(data?.divisions||[],sport,data?.ratingMode||'raw'),rated=all.filter(t=>t.rank);
  ...
  function highlight(row,ratedCount){
    ...
    if(['flag','8u','6u'].includes(row.sport))return null;
  ...
  const favorites={flag:['Burrow','Buccaneers'],'8u':['Pulisic','Arsenal'],'6u':['Messi','Vipers'],followed:{'8u':['Messi','Celtic']}};
  function buildRows(data,sport,today=new Date()){
    ...
    for(const d of (data?.divisions||[]).filter(d=>d.sport===sport))for(const [index,g] of (d.schedule||[]).entries()){
      ...
      const outcomeOnly=g.home_score===null&&g.away_score===null&&['W/L','L/W'].includes(g.home_outcome+'/'+g.away_outcome);
      const completed=outcomeOnly||[g.away_score,g.home_score].every(n=>Number.isInteger(n)&&n>=0);
      ...
      const row={id:JSON.stringify([sport,d.division,index]),sport,division:d.division,away:profile(g.away),home:profile(g.home),
        ...
        outcomeOnly,awayOutcome:g.away_outcome,homeOutcome:g.home_outcome,completed,awayScore:completed?g.away_score:null,homeScore:completed?g.home_score:null,
        status:g.result_status==='unknown'?'Result unknown':completed?'Final':dateISO&&dateISO<today?'Awaiting result':dateISO?'Upcoming':'Date TBD',
        location:g.location,locationUrl:S.safeLocationURL(g.location_url),sourceUrl:S.safeSourceURL(d.url),
        followedTeam:favorites.followed?.[sport]?.[0]===d.division&&[g.away,g.home].includes(favorites.followed[sport][1]),
  ...
  function filterRows(rows,{filter='Upcoming',division='all',query='',watchlist=false,team=null}={}){
    query=query.trim().toLowerCase();
    return rows.filter(r=>(!team||(r.sport===team.sport&&r.division===team.division&&[r.away.team,r.home.team].includes(team.team)))&&(division==='all'||r.division===division)&&(!watchlist||r.highlight)&&
      (filter==='All'||(filter==='Results'?r.completed:!r.completed&&!['Awaiting result','Result unknown'].includes(r.status)))&&
  ...
  function projectionFor(row,payload,now=Date.now()){
    if(row.status==='Result unknown')return null;
    if(['8u','6u'].includes(row.sport)){
      if(row.completed||!Number.isFinite(row.start)||row.start<=now||payload?.model?.sport!==row.sport||payload.model.id!==row.sport+'-gamma-poisson-v1'||payload.model.prior_games!==3)return null;
      return (payload.forecasts||[]).find(f=>{const id=f.fixture_id;return Array.isArray(id)&&id.length===6&&id[0]==='fall-2026'&&id[1]===row.sport&&id[2]===row.division&&id[3]===row.home.team&&id[4]===row.away.team&&Date.parse(id[5])===row.start;})||null;
    }
  ...
  function projectionLine(r,options,detail=false){
    if(!options.showProjections||!['flag','8u','6u'].includes(r.sport))return '';
    const f=projectionFor(r,options.projections,options.now??Date.now());
    if(!f)return !r.completed&&r.status==='Upcoming'?(detail?`<p>Projection unavailable · kickoff passed/unknown, ${r.sport==='flag'?'no division results':'insufficient scored age-group data'}, or no eligible current capture.</p>`:'<span class="league-projection">Model · unavailable</span>'):'';
    if(r.sport!=='flag'){
      const spread=Math.round(Math.abs(f.margin_home)*2)/2,line=spread?`${f.margin_home>0?r.home.team:r.away.team} −${spread}`:'Pick’em';
      if(!detail)return `<span class="league-projection"><b>Model</b> · ${e(line)} · Total ${e(Math.round(f.total*2)/2)} goals</span>`;
      const signed=n=>n>0?'+'+n:String(n),p=options.projections;
      return `<p class="league-projection-detail"><b>Experimental · very small sample</b><br>Home margin ${e(signed(Math.round(f.margin_home*100)/100))} (${e(signed(f.margin_range[0]))} to ${e(signed(f.margin_range[1]))}) · Total ${e(Math.round(f.total*100)/100)} (${e(f.total_range[0])}–${e(f.total_range[1])}) goals.<br>Broad approximate 95% model-based ranges · scored GP away ${e(f.away_gp)} / home ${e(f.home_gp)} · ${e(f.division_games)} division games; ${e(p.pooled_games)} scored ${e(r.sport.toUpperCase())} games pooled.<br>${e(p.model.prior_games)} prior games per attack/concession component; no home advantage or fitted strength-of-opponents adjustment. Ages trained separately; shared baseline across divisions is an assumption.${f.away_gp===0||f.home_gp===0?' 0 scored GP team: prior-based extrapolation.':''} Not calibrated betting odds. <a href="./soccer-age-method.html">Soccer assumptions &amp; archives</a></p>`;
    }
    if(!detail){
  ...
  function renderRows(rows,options={}){
    return groupDates(rows).map(g=>`<section class="league-day"><h3>${g.dateISO?e(new Intl.DateTimeFormat('en-US',{timeZone:'UTC',weekday:'short',month:'short',day:'numeric'}).format(new Date(g.dateISO+'T12:00:00Z'))):'Date TBD'}</h3>${g.rows.map(r=>{
      const identity=t=>`<span class="league-team"><b><span class="league-rank" title="${t.rank?'Power rank '+e(t.rankText)+' · current selected mode':'Unrated · no numerically scored game evidence'}" aria-label="${t.rank?'Power rank '+e(t.rankText)+' · current selected mode':'Unrated · no numerically scored game evidence'}">${t.rank?(t.rankText.startsWith('T')?'T#'+t.rank:'#'+t.rank):'—'}</span> ${e(t.team)}</b> <small>${record(t)}</small></span>`;
      ...
      return `<details class="league-fixture${r.ourTeam||r.followedTeam?' league-fixture-ours':''}" data-league-fixture="${e(r.id)}"><summary title="Expand for coaches, games played and source"><span class="league-time">${e(r.timeLabel)}</span><span class="league-match">${identity(r.away)} <span class="league-at">@</span> ${identity(r.home)}${r.completed?` <b class="league-score${r.outcomeOnly?' league-outcome':''}">${r.outcomeOnly?`${e(r.away.team)} ${r.awayOutcome} – ${e(r.home.team)} ${r.homeOutcome} · score unavailable`:`${r.awayScore}–${r.homeScore}`} Final</b>`:''}${r.status==='Result unknown'?'<b class="league-score league-outcome">Result unknown — source lists L/L</b>':''}${projectionLine(r,options)}</span><span class="league-meta">${e(r.division)} · ${map}${r.ourTeam?' · <b class="league-ours">Our team</b>':r.followedTeam?' · <b class="league-ours">Followed team</b>':''}${r.highlight?` · <b class="league-badge">${e(r.highlight.label)}</b>`:''}${r.status==='Awaiting result'?' · Awaiting result':''}<span class="league-expand" aria-hidden="true"> ▾</span></span></summary><div class="league-detail">${lastPregameDetail(r,options.pregameResults)}${projectionLine(r,options,true)}${[r.away,r.home].map(t=>`<p><b>${e(t.team)}</b> · Coach: ${e(t.coach||'Not listed')} · ${t.gp??'—'} GP${t.outcome_only_gp?` · ${t.scored_gp} scored games; ${t.outcome_only_gp} score unavailable`:''}${t.unknown_gp?` · Verified game record ${record(t)} (${t.gp} GP); Official reported record ${record(t.reported_record)} (${t.reported_record.gp} GP). Coverage excludes 1 unknown result.`:''}${t.gp===0?' · Unrated':t.gp<3?' · Small sample (fewer than 3 GP)':''}</p>`).join('')}${r.highlight?`<p>${e(r.highlight.reason)}</p>`:''}<p>${r.outcomeOnly?'Official W/L; score unavailable. Excluded from scoring and margin models.':r.completed?'Final score shown away–home.':'No final score published.'} ${r.sourceUrl?`<a href="${e(r.sourceUrl)}" target="_blank" rel="noopener noreferrer">Official schedule</a>`:''}</p></div></details>`;
    }).join('')}</section>`).join('')||'<p class="league-empty">No games match these filters. Try All or turn off Watchlist.</p>';
  }
  function projectionGuide(payload,sport='flag'){
    if(['8u','6u'].includes(sport))return `<p class="league-note" data-projection-caution hidden><b>Experimental · very small sample.</b> Model estimates, not market lines or calibrated betting odds. Broad uncertainty; tap a matchup for ranges &amp; scored GP.</p><details class="league-method" data-projection-guide hidden><summary>Projection assumptions &amp; archive</summary><p>The favorite is shown minus a spread, rounded zero is Pick’em. Spread and combined total are rounded to half goals. Broad approximate 95% model ranges are assumptions, not calibrated coverage.</p><p>8U and 6U are trained separately, each pooling its own divisions. Numeric scored games only; outcome-only W/L excluded. Gamma–Poisson attack/concession rates use ${e(payload?.model?.prior_games??3)} prior games, then average own attack and opponent concession. No fitted strength-of-opponents adjustment, home advantage, or measured cross-division strength. Zero scored GP is prior-based extrapolation; small samples are prior-heavy.</p><p>At least four scored games in this age and a positive pooled mean required. Known, wholly unplayed kickoffs must be more than 30 minutes after actual accepted collection; lines hide at kickoff. ${payload?`Calculation ${e(payload.generated_at)}; underlying capture ${e(payload.captured_at)} (UTC). ${e(payload.withheld||'')}`:'Current age projections unavailable.'} Capture time is not publication time. No retroactive pregame claims.</p><a href="./soccer-age-method.html">Full soccer assumptions &amp; age archives</a> · <a href="./soccer-projections-${e(sport)}/index.json">This age’s capture index</a></details>`;
  ...
  function render(data,container,options={}){
    ...
    const selection=options.team?JSON.stringify(options.team)+':'+(options.selectionVersion||0):null;
    if(selection!==state.selection){state.selection=selection;state.filter=selection?'Results':'Upcoming';state.watchlist=false;}
    const activeSport=options.sport||'flag',payload=activeSport==='flag'?data.projections:data.soccerAgeProjections?.[activeSport];
    const rows=buildRows(data,activeSport);
    const update=()=>{
      const shown=filterRows(rows,{...options,...state});
      container.querySelector('[data-league-content]').innerHTML=renderRows(shown,{showProjections:state.showProjections,projections:payload,pregameResults:data.pregameResults});
    ...
    container.innerHTML=`<div class="league-heading"><h2>League schedule</h2><span>Fall 2026 · All dates &amp; times Central</span></div><div class="league-toolbar">${options.team?`<span class="league-selected-team">${e(options.team.team)} · ${e(options.team.division)} · ${e(options.team.sport.toUpperCase())} <a href="#league-schedule">Clear team</a> · <a href="#boardtitle">Back to rankings</a></span>`:''}<div role="group" aria-label="League schedule view">${['Upcoming','Results','All'].map(f=>`<button data-league-filter="${f}">${f}</button>`).join('')}</div><button data-league-watch>Watchlist</button>${['flag','8u','6u'].includes(activeSport)?'<button data-league-projections aria-pressed="false">Experimental projections</button>':''}</div>${['flag','8u','6u'].includes(activeSport)?projectionGuide(payload,activeSport):''}<p class="league-note" data-league-count role="status"></p><p class="league-note">Away @ home · # = ${data.ratingMode||'raw'} power rank · T# = tied; — = unrated · W–L–T. Latest ranks, not kickoff ranks. Disconnected comparisons are provisional, even within divisions.</p><details class="league-method"><summary>Highlights &amp; ranking guide</summary><p>Highlights describe current records, not predictions or playoff stakes. No playoff or tiebreak simulation. Both teams need at least 3 games played and a rated rank; undated, completed and past unscored games receive no badge.</p><p><b>Top matchup</b>: both competition ranks ≤ ceil(rated teams ÷ 4), across all divisions of this sport/age. Ties at the cutoff are included, so more than a quarter can qualify. The denominator includes rated teams with 1–2 GP, but they cannot earn a badge.</p>${['flag','8u','6u'].includes(activeSport)?'<p>Watchlist shows only upcoming Top matchups; it does not reorder them.</p>':'<p><b>Close records</b>: same division, win-rate gap ≤15 percentage points; not a prediction of a close game. Win rate = (W + ½T) / GP. Watchlist shows only upcoming highlighted games; it does not reorder them.</p>'}<p>Ranks use joint opponent-adjusted power with ridge shrinkage and game caps (±21 flag, ±3 soccer), using the selected Raw (default) or capped inputs, independent of filters. Raw has no blowout cap or outlier downweighting. Disconnected result groups share an assumed baseline, not measured relative strength. T = tied competition rank. All records and ranks are latest season totals, not historical pregame stats; GP 0 is unrated, GP 1–2 is a small sample. No division-strength adjustment. Same-day unscored games remain upcoming; older unscored games are awaiting results in All, never assumed draws. Undated fixtures sort last; unknown times precede known times on a date.</p></details><div data-league-content></div>`;
