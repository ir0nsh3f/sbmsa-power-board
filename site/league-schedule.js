(function(root,factory){
  ...
  function projectionLine(r,options){
    if(!options.showProjections||r.sport!=='flag')return '';
    const f=projectionFor(r,options.projections,options.now??Date.now());
    if(!f)return !r.completed&&r.status==='Upcoming'?'<span class="league-projection">Projection unavailable · kickoff passed/unknown, no division results, or no current capture.</span>':'';
    const signed=n=>n>0?'+'+n:String(n);
    return `<span class="league-projection"><b>Experimental · very small sample</b><span>Home margin ${e(signed(f.margin_home))} (${e(signed(f.margin_range[0]))} to ${e(signed(f.margin_range[1]))}) · Total ${e(f.total)} (${e(f.total_range[0])}–${e(f.total_range[1])})</span><small>Approx. 95% model-based ranges · GP away ${e(f.gp_away)} / home ${e(f.gp_home)} · ${e(f.division_games)} division games</small></span>`;
  }
