...
(async()=>{
 ...
 try{
  ...
  for(const width of [320,390,768,1400]){
   ...
   for(const sport of ['flag','8u','6u']){
    ...
    for(const division of divisions)for(const filter of ['Upcoming','Results','All']){
     ...
     assert.equal(await page.locator('[data-league-fixture]').count(),expected,`${width}/${sport}/${division}/${filter}`);counts[`${sport}/${division}/${filter}`]=expected;
    ...
    assert.equal(await page.locator('[data-league-fixture]').count(),coachCount);await page.locator('#search').fill('');
    ...
    if(process.env.QA_DIR){await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:path.join(process.env.QA_DIR,`league-${sport}-${width}.png`)});}
   }
  }
  ...
  for(const width of [320,390,768,1400]){
   await page.setViewportSize({width,height:1000});assert.equal(await page.locator('.league-badge').innerText(),'Top matchup');
