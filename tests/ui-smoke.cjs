...
(async()=>{
 ...
 const server=http.createServer((req,res)=>{
  ...
  const name=['rainbow.js','soccer-method.html','power-method.html','schedule-links.js','ratings.js','data.json','advanced.js','schedules.js','league-schedule.js','league-schedule.css'].includes(requested)?requested:'index.html';
 ...
 try {
   ...
   for(const filter of ['Upcoming','Results','All'])for(const team of ['all','flag|Burrow|Buccaneers','8u|Pulisic|Arsenal','6u|Messi|Vipers']){
    ...
    for(const layout of ['guide','glance']){
     ...
     if(layout==='glance'){
      ...
      const actual=await guide.locator('.glance-fixture').evaluateAll(es=>es.map(e=>e.querySelector(':scope > strong').lastChild.textContent.trim()+'|'+e.querySelector(':scope > span:not(.glance-time) strong').lastChild.textContent.trim()+'|'+(e.parentElement.cellIndex===1?'Dexter':'Beckham')));
  ...
  await guide.locator('[data-schedule-layout="guide"]').click();
  assert.equal(await guide.locator('.fixture-match .schedule-rank').count(),2*await guide.locator('[data-fixture-team]').count());
