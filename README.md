...
## Outcome-only official results (review 2026-11-30)

An exact paired `W` / `L` (either home/away order) is a final result with `home_outcome` / `away_outcome` and **null numeric scores**. It counts in official W/L/GP, comparison points and record-based SOS. It does not imply a forfeit or a score. Partial numbers, mixed numeric/outcome labels, unmatched or contradictory markers still reject the entire collection.

Scoring totals include only numeric results. Affected teams carry `scored_gp` and `outcome_only_gp`; old numeric-only records omit these additive fields (`scored_gp = gp`). Scoring averages, clean-sheet rates and raw/capped margins use scored GP, not total GP. Outcome-only games are excluded from power, adjusted-margin inputs, offense/defense training and numeric forecast evaluation, and cannot receive a future forecast. Zero scored games means unrated/unavailable, not zero strength. Raw ridge3 and the separate projection/prior3 parameters are unchanged.

Schedules label the winner and loser with **score unavailable**; standings and Advanced disclose scored-game coverage. Corrections append a new accepted history snapshot with the paired outcomes and coverage fields on the public allowlist. All previous histories, model captures and publication receipts remain immutable and reproducible; no history is backdated. Source standings GF/GA, when supplied, still require exact reconciliation to known numeric scores: unexplained accounting differences fail closed for source review, never become an invented score. The September 16 Haaland desktop standings supply W/L/T/GP/PTS, **not GF/GA**. Its September 12 Blue Jays at San Jose fixture now says away W / home L; the official classification beyond W/L is unknown.

## Observed season history

Index schema 2 has a ranking-method registry and `ranking_method_id` per capture. Old immutable v1 record-first and v2 capped-power captures and their original ranks are preserved byte-for-byte; v3 explicitly specifies Raw default and optional cap mode. First accepted source check after any method transition appends a `ranking-method-change` baseline even with unchanged results. Never report this as movement. Future charts must split at method boundaries and must not carry observations across them; no chart currently bridges these methods. Unknown historical weeks remain unknown.
...
Default `opponent-ridge-margin-v3` jointly minimizes `½ Σ((home−away) − (r_home−r_away))² + 3/2 Σ r²` independently for each sport/age, using FULL RAW final margins. No automatic blowout cap, Huber loss or residual downweighting. Optional cap mode alone uses clip(home−away, ±21 flag / ±3 soccer). Ratings have point/goal strength-effect units, not average own-result units. W–L–T never drives sorting. GP0 is unrated. Raw is selected on fresh page loads. The cap toggle recalculates all rank consumers together; history records the explicitly versioned Raw default. Team-rating ridge shrinkage is not margin clipping. See [exact objective, priors, solver and limitations](site/power-method.html).
...
The **Schedule** tab uses the same JV Flag / 8U / 6U league buttons, division selector and team/official-coach search. It includes every official fixture in the eight collected divisions, grouped chronologically by Central date; completed scores are **away–home**. Default is the complete Upcoming list, not a highlight-only feed. Results shows numeric finals and validated paired W/L-only results; All also shows past games awaiting results. Unscored same-day games remain upcoming; unknown dates sort last and unknown times first within a date. Original source maps are preserved. Expand a row for both coaches, GP, sample status, badge reason and official source. Our team markers use exact sport/division/team identities. Division filtering can focus on our team's actual divisional opponents without implying playoff relevance.

`site/league-schedule.js` is a pure UMD model plus scoped renderer; `league-schedule.css` is scoped to the new view. It reuses schedule date, escaping and safe-URL helpers. Ranks always use **opponent-adjusted full sport/age power competition ranks**, using the selected Raw/capped mode, independent of filters; ties use the same 8-decimal rounding as Rankings. Unplayed teams are unrated. Records/ranks are current totals, never reconstructed historical pregame stats.
...
- **Top matchup**: both power competition ranks ≤ `ceil(N/4)`, where N is all rated teams in this sport/age, including rated 1–2 GP teams. Ties at the cutoff are included, potentially expanding the qualifying set beyond a quarter. GP 1–2 teams count in N but cannot themselves earn a badge.
- Boys Schedule and Watchlist highlight only Top matchups, not close records. Exact Our team and followed-team warm fills remain independent of badge eligibility. The shared renderer retains girls close-record behavior.
